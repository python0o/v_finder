...
def render_health_panel(con: duckdb.DuckDBPyConnection) -> None:
    ...
    if _table_exists("county_scores") and _table_exists("county_ref") and _table_exists("ppp_clean"):
        df_top = _safe_df(
            """
        ...
        if not df_top.empty:
            ...
            st.info("No rows returned for Top Counties (Risk + PPP).")
    else:
        st.info("Missing one of: county_scores, county_ref, ppp_clean. Skipping top county join.")
    ...
    if _table_exists("ppp_clean"):
        ...
        if not df_lenders.empty:
            st.dataframe(df_lenders, use_container_width=True)
        else:
    ...
    for tbl in snapshot_tables:
        ...
        try:
            ...
            st.dataframe(df[["name", "type", "notnull"]], use_container_width=True, height=210)
        except Exception:
            pass
