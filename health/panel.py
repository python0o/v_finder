...
def render_health_panel(con: duckdb.DuckDBPyConnection) -> None:
    ...
    # -----------------------------
    # PPP → GEOID mapping health (optional)
    # -----------------------------
    mapped_geoids = 0
    mapped_ppp_rows = 0
    ...
    # -----------------------------
    last_ingest = (
        _max_ts("ppp_ingest_log", ["finished_at", "completed_at", "ended_at", "run_ts", "ts"])
        or _max_ts("ppp_ingest_log", ["created_at", "ingest_ts"])
        or None
    )
    ...
    df_counts = pd.DataFrame(
        [{"table": label, "rows": _count(tbl) if _table_exists(tbl) else 0} for tbl, label in core_tables]
    )
    ...
    with right:
        if _table_exists("county_scores"):
            ...
            if not df_scores.empty:
                df_scores["bucket"] = pd.to_numeric(df_scores["risk_score"], errors="coerce").fillna(0).round(0)
                st.bar_chart(df_scores["bucket"].value_counts().sort_index())
                st.bar_chart(df_scores["risk_tier"].fillna("UNKNOWN").value_counts())
            else:
                st.info("county_scores has no non-null risk_score values.")
        else:
            st.info("county_scores not found; skipping risk charts.")

    # -----------------------------
    # Top Counties (Risk-first, PPP optional)
    # -----------------------------
    st.markdown("---")
    ...
    if _table_exists("county_scores") and _table_exists("county_ref"):
        ...
        st.info("Required analytics tables not found; skipping Top Counties.")

    # -----------------------------
    # Top lenders (PPP only, optional)
    # -----------------------------
    st.markdown("---")
